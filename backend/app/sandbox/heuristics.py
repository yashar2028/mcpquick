...
def build_heuristic_result(
    ...
    latency_efficiency = clamp01(1.0 - (latency_ms / 12000.0))
    cost_efficiency = clamp01(1.0 - (max(total_tokens - 600, 0) / 5000.0))
    step_efficiency = clamp01(1.0 - (1.0 / max(max_steps, 1)) * 0.25)

    verbosity_factor = clamp01((normalized_output_len / normalized_prompt_len) * 0.8)
    reliability_recovery = clamp01(0.55 + 0.35 * verbosity_factor)
